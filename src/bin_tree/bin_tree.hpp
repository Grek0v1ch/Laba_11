...
/* Реализыция АВЛ-дерева. Дерево состоит из узлов, кождый из который хранит в себе значение (ключ)
   и ссылки на своих детей. Класс шаблонный, для работы с пользовательским типом данных необхадимо
   ...
   Основная информация для написания дерева взята с сайтов:
   1. Дерево поиска, наивная реализация. -
      https://neerc.ifmo.ru/wiki/index.php?title=Дерево_поиска,_наивная_реализация
   2. АВЛ-деревья. Реализация. -
...
pnode<Type> bin_tree<Type>::balance(pnode<Type> pos) {
	fix_height(pos);
	/* Следует понимать, что разность в высотах поддеревьев у узла никога не будет больше 2 по
	   модулю.
